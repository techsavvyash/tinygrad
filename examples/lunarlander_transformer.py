...
class DecisionTransformer:
  def __init__(self, state_space, action_space, dim=192, n_heads=12, n_layers=2, norm_eps=1e-5, max_seq_len=1024):
    self.dim = dim
    self.s_embedding = nn.Linear(state_space, dim//3)
    self.a_embedding = nn.Embedding(action_space+2, dim//3)
    self.R_embedding = nn.Linear(1, dim//3)
...
def rollout(env, model):
  ...
  while not terminated and not truncated:
    ...
    act = sample(a_logits[:, -1, :], 0.7).item()
...
if __name__ == "__main__":
  ...
  while 1:
    ...
    first = True
    while len(all_R) < BS or first:
      R,s,a = rollout(env, model)
      returns.append(R[0])
      R,s,a = clip_r(R,s,a,2)
